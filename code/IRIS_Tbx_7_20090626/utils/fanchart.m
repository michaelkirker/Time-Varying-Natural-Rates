...
if ~strcmp(get(h0,'type'),'line'), error('Incorrect input handle.'); end
...
if istseries(bands)
  ...
  bands = transpose(double(bands,range));
else
  if length(bands) ~= ndata, error('Incorrect length of fanchart data.'); end
  if size(bands,2) < size(bands,1), bands = transpose(bands); end
end
